...
public final class BidirectionalStream implements UnidirectionalDataOutput, UnidirectionalDataInput {

    private final int id;
    private final @NotNull DataOutputStream output;
    private final @NotNull DataInputStream input;

    BidirectionalStream(@NotNull QuicStreamImpl quicStream) {
        if (!quicStream.isBidirectional()) {
            throw new IllegalArgumentException("The quick stream must to be bidirectional and self initialized");
        }
