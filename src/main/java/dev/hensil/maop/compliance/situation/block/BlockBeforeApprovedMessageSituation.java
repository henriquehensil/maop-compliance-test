...
final class BlockBeforeApprovedMessageSituation extends Situation {
    ...
    public boolean diagnostic() {
        try {
            ...
            log.info("Serializing both operations");
            byte @NotNull [] messageBytes = message.toBytes();
            byte @NotNull [] blockBytes = block.toBytes();

            byte @NotNull [] arr = ByteBuffer.allocate(messageBytes.length + blockBytes.length)
                    .put(messageBytes)
                    .put(blockBytes)
                    .array();

            log.info("Writing message operation with " + bytes.length + " payload and block operation");
            stream.write(arr);
